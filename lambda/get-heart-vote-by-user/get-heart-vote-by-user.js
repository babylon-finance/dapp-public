const { getHeartDistVoteByUser } = require('../shared/queries.js');
const { verifyDomain } = require('../shared/Verify.js');

const FUNCTION_NAME = 'get-heart-vote-by-user';
...
exports.handler = async function (event, context, callback) {
  ...
  try {
    ...
    const result = await getHeartDistVoteByUser(address);
    const finalResult = result ? result.data.map((d) => d.data) : [];
